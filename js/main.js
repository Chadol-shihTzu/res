$(document).ready(function(){
    ...
    //윈도우의 가로길이가 1280미만이면 모바일 버전 메뉴가 나타남.
    if($(window).width() >= 1280){
        //pc버전
        //주메뉴에 마우스오버하면 서브메뉴와 배경나타남
        $('nav > ul > li').hover(function(){
            $('.sub').stop().slideDown(0);
            $('.sub_bg').addClass('active');
            $('header').addClass('minimize');
            $('.community .sub').addClass('active');
        }); 
        //haeder에서 마우스 아웃하면 서브메뉴와 배경사라짐
...
function minimize_header() {
  ...
  //함수 호출
    ...
    const swiper2 = new Swiper('.highlight', {
        slidesPerView:1.5,
        spaceBetween:30,
        autoplay:{
            delay:3000,
    ...
    $(window).scroll(function(){
        ...
        $('.scrollSet').each(function(){
            //$(this) : .scrollSet(인덱스 0~4 0,1,2,3,4 5개)
            //offset().top : section의 top값
            let secTop=$(this).offset().top-200;
            //section의 bottom값
            ...
            } //내릴때 오릴때 둘 다 애니메이션 되었으면 좋겠다싶으면 else하고 removeClass(active) 하면됨
        });
    });
